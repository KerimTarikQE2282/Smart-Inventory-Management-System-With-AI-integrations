...
export const useWarehouseData = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await axios.get('https://seniorproject-inventory-managemnet-system.onrender.com/api/v1/WareHouse/');
          ...
          warehousesArray.map(async (warehouse) => {
            try {
              const response = await axios.get(
                `https://seniorproject-inventory-managemnet-system.onrender.com/api/v1/WareHouse/wareHouseItemsDetailed/${warehouse._id}`
              );
