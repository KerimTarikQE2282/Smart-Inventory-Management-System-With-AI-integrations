...
export default function DataTable({ name, columns = [''], resourceTitle }) {
  ...
  const exportJson = () => {
    const fileName = 'WareHouse_data_export';
                ...
                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={mydata._id}>
                  {columns.map((title, i) => (
                    <td
                      ...
                      onClick={() => router.replace(`/storing/WareHouse/details/${mydata._id}`)} // Fix route.replace() to router.replace()
                    >
                      {title.includes(".") ? (
                  ...
                  <td className="py-4 flex gap-10">
                    <Link href={`/storing/${resourceTitle}/update/${mydata._id}`} className="font-medium text-blue-500">
