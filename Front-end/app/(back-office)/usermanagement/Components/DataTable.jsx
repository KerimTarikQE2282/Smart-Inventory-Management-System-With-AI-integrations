...
export default function DataTable({ name, columns = [''], resourceTitle }) {
  ...
  const onSubmit = async (formData) => {
    ...
    const baseUrl = `http://https://seniorproject-inventory-managemnet-system.onrender.com/api/v1//${name}`;
            ...
            {Data?.map((mydata) => (
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={mydata.title}>
                ...
                <td className="py-4 flex gap-10">
                  <Link href={`/usermanagement/${resourceTitle}/update/${mydata._id}`} className='font-medium text-blue item-center space-x-2 text-blue-500'>
